...
function dispatchFn(action: Action) {
...
function toast({ ...props }: Toast) {
  ...
  const update = (props: Partial<ToasterToast>) =>
    dispatchFn({ type: "UPDATE_TOAST", toast: { ...props, id } });
  const dismiss = () => dispatchFn({ type: "DISMISS_TOAST", toastId: id });
...
function useToast() {
  ...
  return {
    ...state,
    toast,
    dismiss: (toastId?: string) => dispatchFn({ type: "DISMISS_TOAST", toastId }),
  };
}
